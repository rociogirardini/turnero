...
function validar(str1, str2, str3, str4, str5) {
  ...
  $("#alerta").append(
    "<p class='alertText'>No todos los campos se han completado correctamente</p>"
  );
  alertas(1, "alerta");
  return false;
}
...
function fncEnviar() {
  ...
  if (validacion) {
    ...
    imprimir.innerHTML =
      '<input type="button" value="Imprimir" class="btn btn-primary" onclick="javascript:window.print()" />';
  }
  ...
  if (validar(text_name, numberID, inputState, textFecha, textHora)) {
    ...
    if (miTurnero.addTurno(inputState, textFecha, textHora)) {
      ...
      $.post(URLGET, text_name, (respuesta, estado) => {
        if (estado === "success") {
          $("#envioExitoso").prepend(
            `<p>¡Gracias ${text_name}! Tu turno se confirmó correctamente</p>`
          );
      ...
      $("#alerta").append(
        "<p class='alertText'>La hora seleccionada está ocupada</p>"
      );
      alertas(1, "alerta");
    }
  }
...
$(document).ready(function () {
  ...
  for (i = 1; i <= imgItems; i++) {
    $(".pagination").append(
      '<li><span><img src="./media/paginacion.png" width="10px"></span></li>'
    ); //Iteración del ciclo para que se creen tantos como slides haya
  }
  $(".slider li").hide();
  ...
  function nextSlider() {
    ...
    $(".pagination img").css({ width: "10px" }); // Cambiando tamaño únicamente del slider activo
    $("img", ".pagination li:nth-child(" + imgPosition + ")").css({
      width: "12px",
    });
